...
class pdfresults(baseUIClass):
    ...
    def createPDF(self, outputFilename=None):
        ...
        if len(pdfFilename) > 0:
            ...
            goodFilePermissions = True
            try:
                # Test whether the file is writable. If not - raise an exception
                with path.open("r+b"):
                    pass
            except PermissionError:
                self.completeLabel.config(text="Cannot open file for writing. Is it is use?", foreground=msg_errorColor)
                goodFilePermissions = False
            except Exception:
                # Likely the file doesn't exist
                pass

            if goodFilePermissions:
                # Looks OK - create the in-memory PDF image
                ...
                if not batchMode:
                    ...
                    self.completeLabel.config(text="PDF file generation complete.", foreground=msg_completeColor)
